...
public class Grafo {
    ...
    boolean raizDif(int i, int j)
    {
        if(this.Raiz[i]==this.Raiz[j])
            return true;
        else
            return false;
    }
    ...
    protected String Kruskal()
    {
        ...
        /*for(int i=1; i<this.nodos; i++) //recorrer los nodos
        {
            ...
            {
                this.lista[a].CrearArista(i, j, this.adyacencia[i][j]);   //crea un arista con origen i, destino j, de peso adyacencia
                a++;  //aumenta el índice del arreglo de aristas
            }
        ...
        */
        ...
        int peso=0;
        int menor, xmenor, ymenor;
        while(!RaicesIguales()) //repetir mientras haya raíces diferentes
        {
            menor=Integer.MAX_VALUE;//inicializar con un valor máximo
            xmenor=0;  //poner el valor de las coordenadas...
            ymenor=0;  //del nodo de inicio
            for(int i=0; i<nodos; i++)//recorre todos los nodos
            {
                    for(int j=0; j<nodos; j++)  //recorrer todos los nodos
                    {
                        //Si hay arista entre dos nodos, es la arista más corta a considerar
                        //y tienen diferentes raíces (no están conectados aún), hacer
                        if(adyacencia[i][j]!=0 && adyacencia[i][j]<menor && Raiz[i]!=Raiz[j])  
                        {
                            menor=this.adyacencia[i][j];  //guardar el valor de la arista
                            xmenor=i;   //guardar la posición x
                            ymenor=j;   //guardar la posición y
                ...
                CambiarRaiz(this.Raiz[xmenor], this.Raiz[ymenor]);       //Cambiar la raíz del origen por la raíz del destino
                this.prim[xmenor][ymenor]=menor;  //guardar la arista en la matriz
                this.prim[ymenor][xmenor]=menor;  //en ambas posiciones (espejo)
                peso=peso+menor;    //acumular el peso
        }
        regresar=regresar+"\nÁrbol menor:"; //poner en la cadena
        char nodo1 = 'A';//guarda el caracter A
        regresar=regresar+"\t"; //poner en la cadena
        for(int r=0; r<nodos; r++)
        {
            regresar=regresar+"\t "+nodo1;//imprime todos los nodos desde la A
            nodo1++;    //aumenta al siguiente nodo
        }
        regresar=regresar+"\n";
        nodo1='A';  //guarda el caracter A
        for(int r = 0; r<nodos; r++)//imprimir la matriz de adyacencia, recorre las columnas
        {
            regresar=regresar+"\t"+nodo1;    //imprime la letra
            
            for(int j = 0; j<nodos; j++)    //recorre una fila
            {
                regresar=regresar+"\t["+this.prim[r][j]+"]";//imprime el valor en la posicion i,j
        ...
        regresar = regresar + "\nCon un peso de: "+String.valueOf(peso);   //guardar en la cadena
        
        for(int i=0; i< this. nodos; i++)   //reiniciar los valores de la tabla de raíces
        {
            Raiz[i]=i;  //poner el mismo valor en la raíz que el nodo
        }
        
        reiniciarPrim();    //poner en 0 la matriz de prim
        
        peso = 0;   //reiniciar el peso
        
        while(!RaicesIguales()) //repetir mientras haya raíces diferentes
        {
            menor=Integer.MIN_VALUE;//inicializar con un valor máximo
            xmenor=0;  //poner el valor de las coordenadas...
            ymenor=0;  //del nodo de inicio
            for(int i=0; i<nodos; i++)//recorre todos los nodos
            {
                    for(int j=0; j<nodos; j++)  //recorrer todos los nodos
                    {
                        //Si hay arista entre dos nodos, es la arista más larga a considerar
                        //y los nodos tienen diferentes raíces (no están conectados), hacer
                        if(adyacencia[i][j]!=0 && adyacencia[i][j]>menor && Raiz[i]!=Raiz[j])  
                        {
                            menor=this.adyacencia[i][j];  //guardar el valor de la arista
                            xmenor=i;   //guardar la posición x
                ...
                this.prim[xmenor][ymenor]=menor;  //guardar la arista en la matriz
                this.prim[ymenor][xmenor]=menor;  //en ambas posiciones (espejo)
                peso=peso+menor;    //acumular el peso del árbol mayor
        }
        
        regresar=regresar+"\nÁrbol mayor:"; //guardar en la cadena
        nodo1 = 'A';//guarda el caracter A
        regresar=regresar+"\t";
        for(int r=0; r<nodos; r++)
        {
        ...
        reiniciarPrim();    //poner la matriz de prim en 0
        
        regresar = regresar + "\nCon un peso de: "+String.valueOf(peso);   //guardar el peso en la cadena  
        
        return regresar;    //regresar la cadena con el resultado :)
    }
